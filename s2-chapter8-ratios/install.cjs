...
const home=path.join(root,'app/maths/s2/chapter-8/page.tsx'),lesson=path.join(root,'app/maths/s2/chapter-8/ratios/page.tsx'),patch=require('./homepage-patch.json');
...
const data=fs.readFileSync(path.join(__dirname,'files/page.tsx.txt'),'utf8');if(fs.existsSync(lesson)&&fs.readFileSync(lesson,'utf8')!==data)throw Error('Ratios page already exists and differs. No files changed.');
if(original===next&&fs.existsSync(lesson)){console.log('Already installed.');process.exit(0)}
const backup=path.join(root,'chapter8-ratios-backup-'+Date.now());fs.mkdirSync(backup);fs.writeFileSync(path.join(backup,'homepage.tsx.bak'),original);const existed=fs.existsSync(lesson);
...
console.log('Installed Ratios and enabled homepage link. Backup: '+backup+'\nRun npm run build.');
