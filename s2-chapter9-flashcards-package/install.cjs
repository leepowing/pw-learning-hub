/* From pw-learning-hub: node s2-chapter9-flashcards-package/install.cjs */
'use strict';
...
const sourceFiles = [
  'data/maths/flashcards/s2chapter9.ts',
  'components/maths/S2Chapter9FlashcardDiagram.tsx',
  'app/maths/s2/chapter-9/flashcards/page.tsx',
];
...
function main() {
  read('package.json');
  read('components/maths/S2Chapter9Geometry.tsx');
  ...
  let s2 = read(s2Path);
  if (!s2.includes('s2Chapter9Flashcards')) {
    if (/chapter\s*:\s*9\b|["']s2-c9-/.test(s2)) fail('Chapter 9 cards already exist in s2.ts. Review their IDs before adding this set. No files changed.');
    s2 = once(s2, /export\s+const\s+s2Flashcards\s*:\s*MathsFlashcard\[\]\s*=\s*\[/g, 'export const s2Flashcards: MathsFlashcard[] = [\n  ...s2Chapter9Flashcards,', 's2Flashcards array');
    s2 = 'import { s2Chapter9Flashcards } from "./s2chapter9";\n' + s2;
    propose(s2Path, s2);
  } else if (!/\.\.\.s2Chapter9Flashcards\b/.test(s2) || !/from\s*["']\.\/s2chapter9["']/.test(s2)) fail('Incomplete Chapter 9 registry integration. No files changed.');

  const componentPath = 'components/maths/FormulaFlashcards.tsx';
  let component = read(componentPath);
  const marker = '// S2 Chapter 9 diagrams (additive integration).';
  if (!component.includes(marker)) {
    if (component.includes('s2Chapter9Diagram') || component.includes('S2Chapter9FlashcardDiagram')) fail('A different Chapter 9 diagram integration already exists. No files changed.');
    ...
    component = once(component, /(["']use client["'];)/g, '$1\n\n' + marker + '\nimport S2Chapter9FlashcardDiagram, { type S2Chapter9DiagramKind } from "@/components/maths/S2Chapter9FlashcardDiagram";\n', 'client directive');
    component = once(component, /s2Chapter6Diagram\?:\s*S2Chapter6DiagramKind;/g, '$&\n  s2Chapter9Diagram?: S2Chapter9DiagramKind;', 'Chapter 6 visual type');
    component = once(component, /const hasFrontVisual = Boolean\(([^;]*?)\);/g, 'const hasFrontVisual = Boolean($1 || currentCard.s2Chapter9Diagram);', 'front visual detection');
    component = once(component, /\{currentCard\.s2Chapter6Diagram && <S2Chapter6FlashcardDiagram kind=\{currentCard\.s2Chapter6Diagram\} \/>\}/g, '$&\n            {currentCard.s2Chapter9Diagram && <S2Chapter9FlashcardDiagram kind={currentCard.s2Chapter9Diagram} />}', 'Chapter 6 diagram render position');
    component = once(component, /const useContentSizedFaces = currentCard\.level === ["']s2["'] && \(currentCard\.chapter === 6 \|\| currentCard\.chapter === 7 \|\| currentCard\.chapter === 8\);/g, 'const useContentSizedFaces = currentCard.level === "s2" && (currentCard.chapter === 6 || currentCard.chapter === 7 || currentCard.chapter === 8 || currentCard.chapter === 9);', 'content-sized Chapter 6 faces');
    component = component.replace(/\bisS2Chapter6\b/g, 'useContentSizedFaces');
    propose(componentPath, component);
  } else {
    for (const expected of ['s2Chapter9Diagram?: S2Chapter9DiagramKind;', '<S2Chapter9FlashcardDiagram kind={currentCard.s2Chapter9Diagram} />', 'currentCard.s2Chapter9Diagram);', 'currentCard.chapter === 6 || currentCard.chapter === 7 || currentCard.chapter === 8 || currentCard.chapter === 9', 'display: useContentSizedFaces ? "grid"']) {
      if (!component.includes(expected)) fail('Incomplete Chapter 9 component integration. No files changed.');
    }
  }

  const homePath = 'app/maths/s2/chapter-9/page.tsx';
  let home = read(homePath);
  if (!home.includes('/maths/s2/chapter-9/flashcards')) {
    home = once(home, /<span className="coming">Coming after the checkpoint<\/span>/g, '<Link className="back" href="/maths/s2/chapter-9/flashcards">Start Flashcards →</Link>', 'Chapter 9 homepage flashcards button');
  ...
  if (!changes.size) { console.log('S2 Chapter 9 Flashcards are already installed. No files changed.'); return; }
  if (dryRun) { console.log('Preflight passed. Would update:'); for (const relative of changes.keys()) console.log('  ' + relative); return; }

  const backup = path.join(root, 'flashcards-backup-s2-c9-' + new Date().toISOString().replace(/[:.]/g, '-'));
  ...
  console.log('Installed S2 Chapter 9: 36 flashcards and 7 SVG diagram variants.');
  ...
  console.log('Then open /maths/s2/chapter-9/flashcards and check one saved answer while signed in.');
}
