...
try{
 ...
 const dest='components/maths/Chapter9Geometry.tsx',full=path.join(root,dest),next=fs.readFileSync(path.join(__dirname,'files/Chapter9Geometry.tsx.txt'),'utf8'),old=fs.existsSync(full)?fs.readFileSync(full,'utf8'):null;
 if(old!==null&&old!==next)throw Error('Existing Chapter9Geometry.tsx differs; no files changed.');
 if(old!==next)changes.push({dest,old,next});
 ...
 const backup=path.join(root,'chapter9-svg-backup-'+Date.now()),done=[];
 ...
 console.log('Installed Chapter 9 SVG diagrams. Backup: '+backup+'\nNext: npm run build');
