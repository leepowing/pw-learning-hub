...
const home=path.join(root,'app/maths/s2/chapter-9/page.tsx'),lesson=path.join(root,'app/maths/s2/chapter-9/similar-plane-figures/page.tsx'),patch=require('./homepage-patch.json');
...
const data=fs.readFileSync(path.join(__dirname,'files/page.tsx.txt'),'utf8');if(fs.existsSync(lesson)&&fs.readFileSync(lesson,'utf8')!==data)throw Error('Section 9.4 page already exists and differs. No files changed.');
...
console.log('Installed Section 9.4 and enabled homepage link. Backup: '+backup+'\nRun npm run build.');
