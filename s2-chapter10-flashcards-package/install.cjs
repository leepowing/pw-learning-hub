/* From pw-learning-hub: node s2-chapter10-flashcards-package/install.cjs */
'use strict';
...
const sourceFiles = [
  'data/maths/flashcards/s2chapter10.ts',
  'components/maths/S2Chapter10FlashcardDiagram.tsx',
  'app/maths/s2/chapter-10/flashcards/page.tsx',
];
...
function main() {
  read('package.json');
  read('components/maths/S2Chapter10Geometry.tsx');
  ...
  let s2 = read(s2Path);
  if (!s2.includes('s2Chapter10Flashcards')) {
    if (/chapter\s*:\s*10\b|["']s2-c10-/.test(s2)) fail('Chapter 10 cards already exist in s2.ts. Review their IDs before adding this set. No files changed.');
    s2 = once(s2, /export\s+const\s+s2Flashcards\s*:\s*MathsFlashcard\[\]\s*=\s*\[/g, 'export const s2Flashcards: MathsFlashcard[] = [\n  ...s2Chapter10Flashcards,', 's2Flashcards array');
    s2 = 'import { s2Chapter10Flashcards } from "./s2chapter10";\n' + s2;
    propose(s2Path, s2);
  } else if (!/\.\.\.s2Chapter10Flashcards\b/.test(s2) || !/from\s*["']\.\/s2chapter10["']/.test(s2)) fail('Incomplete Chapter 10 registry integration. No files changed.');

  const componentPath = 'components/maths/FormulaFlashcards.tsx';
  let component = read(componentPath);
  const marker = '// S2 Chapter 10 diagrams (additive integration).';
  if (!component.includes(marker)) {
    if (component.includes('s2Chapter10Diagram') || component.includes('S2Chapter10FlashcardDiagram')) fail('A different Chapter 10 diagram integration already exists. No files changed.');
    ...
    component = once(component, /(["']use client["'];)/g, '$1\n\n' + marker + '\nimport S2Chapter10FlashcardDiagram, { type S2Chapter10DiagramKind } from "@/components/maths/S2Chapter10FlashcardDiagram";\n', 'client directive');
    component = once(component, /s2Chapter6Diagram\?:\s*S2Chapter6DiagramKind;/g, '$&\n  s2Chapter10Diagram?: S2Chapter10DiagramKind;', 'Chapter 6 visual type');
    component = once(component, /const hasFrontVisual = Boolean\(([^;]*?)\);/g, 'const hasFrontVisual = Boolean($1 || currentCard.s2Chapter10Diagram);', 'front visual detection');
    component = once(component, /\{currentCard\.s2Chapter6Diagram && <S2Chapter6FlashcardDiagram kind=\{currentCard\.s2Chapter6Diagram\} \/>\}/g, '$&\n            {currentCard.s2Chapter10Diagram && <S2Chapter10FlashcardDiagram kind={currentCard.s2Chapter10Diagram} />}', 'Chapter 6 diagram render position');
    component = once(component, /const useContentSizedFaces = currentCard\.level === ["']s2["'] && \(currentCard\.chapter === 6 \|\| currentCard\.chapter === 7 \|\| currentCard\.chapter === 8 \|\| currentCard\.chapter === 9\);/g, 'const useContentSizedFaces = currentCard.level === "s2" && (currentCard.chapter === 6 || currentCard.chapter === 7 || currentCard.chapter === 8 || currentCard.chapter === 9 || currentCard.chapter === 10);', 'content-sized Chapter 6 faces');
    component = component.replace(/\bisS2Chapter6\b/g, 'useContentSizedFaces');
    propose(componentPath, component);
  } else {
    for (const expected of ['s2Chapter10Diagram?: S2Chapter10DiagramKind;', '<S2Chapter10FlashcardDiagram kind={currentCard.s2Chapter10Diagram} />', 'currentCard.s2Chapter10Diagram);', 'currentCard.chapter === 6 || currentCard.chapter === 7 || currentCard.chapter === 8 || currentCard.chapter === 9 || currentCard.chapter === 10', 'display: useContentSizedFaces ? "grid"']) {
      if (!component.includes(expected)) fail('Incomplete Chapter 10 component integration. No files changed.');
    }
  }

  const homePath = 'app/maths/s2/chapter-10/page.tsx';
  let home = read(homePath);
  if (!home.includes('/maths/s2/chapter-10/flashcards')) {
    home = once(home, /<span className="coming">Coming after the checkpoint<\/span>/g, '<Link className="back" href="/maths/s2/chapter-10/flashcards">Start Flashcards →</Link>', 'Chapter 10 homepage flashcards button');
  ...
  if (!changes.size) { console.log('S2 Chapter 10 Flashcards are already installed. No files changed.'); return; }
  if (dryRun) { console.log('Preflight passed. Would update:'); for (const relative of changes.keys()) console.log('  ' + relative); return; }

  const backup = path.join(root, 'flashcards-backup-s2-c10-' + new Date().toISOString().replace(/[:.]/g, '-'));
  ...
  console.log('Installed S2 Chapter 10: 36 flashcards and 5 SVG diagram variants.');
  ...
  console.log('Then open /maths/s2/chapter-10/flashcards and check one saved answer while signed in.');
}
