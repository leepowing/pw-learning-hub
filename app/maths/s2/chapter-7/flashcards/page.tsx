...
import { s2Chapter7Flashcards } from "@/data/maths/flashcards/s2chapter7";

export default function S2Chapter7FlashcardsPage() {
  const [cards, setCards] = useState<typeof s2Chapter7Flashcards | null>(null);
  const [selected, setSelected] = useState<string[]>(Array.from(new Set(s2Chapter7Flashcards.map(card => card.section))));
  const sections = Array.from(new Set(s2Chapter7Flashcards.map(card => card.section)));
  const available = s2Chapter7Flashcards.filter(card => selected.includes(card.section));
  ...
  return <main className="page">
    <Link href="/maths/s2/chapter-7" className="backLink">← Back to Chapter 7</Link>
    <header><p className="eyebrow">S2 · CHAPTER 7 · FLASHCARDS</p><h1>Organization and Presentation of Data (II)</h1><p>Recall the definitions, read the statistical diagrams and explain fair comparisons.</p></header>
    {cards ? <><div className="sessionToolbar"><button type="button" onClick={() => setCards(null)}>← Change sections</button><span>{cards.length} cards in this session</span></div><FormulaFlashcards key={cards.map(card => card.id).join(",")} cards={cards} /></> : <section className="selection">
      ...
      <div className="options">{sections.map((section, index) => <label key={section}><input type="checkbox" checked={selected.includes(section)} onChange={() => setSelected(current => current.includes(section) ? current.filter(item => item !== section) : [...current, section])} /><span><strong>Section {index + 1} · {section}</strong><small>{s2Chapter7Flashcards.filter(card => card.section === section).length} flashcards</small></span></label>)}</div>
      ...
      <div className="links"><Link href="/maths/flashcards?level=s2&chapter=7">Choose across levels and chapters →</Link><Link href="/maths/flashcards/review">Start Smart Review →</Link><Link href="/maths/flashcards/progress">View my progress →</Link></div>
    </section>}
    <style jsx>{`
