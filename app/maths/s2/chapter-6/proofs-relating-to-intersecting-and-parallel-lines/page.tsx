...
const relationships: { kind: Relationship; title: string; location: string; result: string; forwardReason: string; converseReason: string }[] = [
  { kind: "alternate", title: "Alternate angles", location: "Inside the two lines, on opposite sides of the transversal.", result: "a = b", forwardReason: reasonRef("parallel.alternate", {"params":{"line1":"p","line2":"q"},"variant":"textbook1"}), converseReason: reasonRef("parallel.converseAlternate", {"params":{"line1":"p","line2":"q"},"variant":"textbook1"}) },
  { kind: "corresponding", title: "Corresponding angles", location: "In matching positions at the two intersections.", result: "a = b", forwardReason: reasonRef("parallel.corresponding", {"params":{"line1":"p","line2":"q"},"variant":"textbook2"}), converseReason: reasonRef("parallel.converseCorresponding", {"params":{"line1":"p","line2":"q"},"variant":"textbook1"}) },
  { kind: "interior", title: "Interior angles on the same side", location: "Inside the two lines, on the same side of the transversal.", result: "a + b = 180°", forwardReason: reasonRef("parallel.coInterior", {"params":{"line1":"p","line2":"q"},"variant":"textbook1"}), converseReason: reasonRef("parallel.converseCoInterior", {"params":{"line1":"p","line2":"q"},"variant":"textbook1"}) },
];
const questions = [
  { id: "straight", prompt: "Three adjacent angles on one side of a straight line are 2x, x and 3x. What is x?", options: ["30°", "60°", "90°"], answer: 0, explanation: "2x + x + 3x = 180° " + reasonRef("angles.straightLine", {"variant":"textbook1"}) + ". Hence 6x = 180° and x = 30°." },
  { id: "opposite", prompt: "Two straight lines intersect. One of the angles is 137°. Which result follows?", options: ["Every angle at the intersection is 137°.", "Its vertically opposite angle is 137°.", "Each adjacent angle is 137°."], answer: 1, explanation: "Vertically opposite angles are equal. Each adjacent angle is 180° − 137° = 43°." + reasonRef("angles.verticallyOpposite", {"variant":"textbook1"}) + reasonRef("angles.straightLine", {"variant":"textbook1"}) },
  { id: "point", prompt: "The three non-overlapping angles making a complete turn at O are 80°, 125° and x. Find x.", options: ["25°", "205°", "155°"], answer: 2, explanation: "80° + 125° + x = 360° " + reasonRef("angles.aroundPoint", {"variant":"textbook1"}) + ", so x = 155°. All three angles must be counted once." },
  { id: "converse", prompt: "A transversal cuts p and q. A pair of alternate angles are both 62°. What is the correct conclusion and reason?", options: ["p // q, because alternate angles are equal.", "p // q, because angles at a point sum to 360°.", "p is perpendicular to q, because the two angles are equal."], answer: 0, explanation: "Use the converse: equal alternate angles establish that p and q are parallel. Parallelism is the conclusion, not the starting condition." + reasonRef("parallel.converseAlternate", {"params":{"line1":"p","line2":"q"},"variant":"textbook1"}) },
  { id: "interior", prompt: "A transversal cuts two lines. The interior angles on the same side are 73° and 107°. What can you prove?", options: ["The lines are perpendicular.", "The lines are parallel, since 73° + 107° = 180°.", "Nothing: interior angles must be equal to prove parallel lines."], answer: 1, explanation: "The interior angles on the same side are supplementary, so the two lines are parallel (converse of the interior-angle theorem)." + reasonRef("parallel.converseCoInterior", {"params":{"line1":"p","line2":"q"},"variant":"textbook1"}) },
  ...
  return <main className="linesPage">
    ...
    <section className="lessonCard">
      ...
      <div className="factGrid">
        <article className="factTile"><h3>Adjacent angles on a straight line</h3><StraightDiagram /><p className="factEquation">a + b = 180°</p><p>The angles share an arm, and their other arms point in opposite directions along one straight line.</p><p className="reference"><DualReason reasonId="angles.straightLine" variant="textbook1" heading /></p></article>
        <article className="factTile"><h3>Vertically opposite angles</h3><CrossingDiagram /><p className="factEquation">a = c</p><p>The angles face each other at the intersection of two straight lines.</p><p className="reference"><DualReason reasonId="angles.verticallyOpposite" variant="textbook1" heading /></p></article>
        <article className="factTile"><h3>Angles at a point</h3><PointDiagram /><p className="factEquation">a + b + c = 360°</p><p>Include all the non-overlapping angles that make one complete turn, counting each angle once.</p><p className="reference"><DualReason reasonId="angles.aroundPoint" variant="textbook1" heading /></p></article>
      </div>
    ...
    <section className="lessonCard workedCard">
      ...
      <Proof caption="Proof of the vertically opposite angle theorem" rows={[["a + b = 180°", reasonRef("angles.straightLine", {"variant":"textbook1"})], ["b + c = 180°", reasonRef("angles.straightLine", {"variant":"textbook1"})], ["a + b = b + c", "Both expressions equal 180°."], ["∴ a = c", "Subtract b from both sides."]]} />
    ...
    <section className="lessonCard">
      ...
      <details className="solution"><summary>Reveal the perpendicular-line proof</summary><div className="solutionBody"><Proof caption="From an angle equation to perpendicularity" rows={[["2x + x + 3x = 180°", reasonRef("angles.straightLine", {"variant":"textbook1"})], ["6x = 180°", "Collect like terms."], ["x = 30°", "Divide both sides by 6."], ["∠COD = 3x = 90°", "Substitute x = 30° into the given angle expression."], ["∴ OC ⟂ AD", reasonRef("geometry.perpendicular")]]} /></div></details>
    ...
    <section className="lessonCard workedCard">
      ...
      <Proof caption="A theorem first, then a converse" rows={[["∠CDB = ∠CEF = 106°", reasonRef("parallel.corresponding", {"params":{"line1":"BD","line2":"FE"},"variant":"textbook4"})], ["∠ABD + ∠CDB = 74° + 106°", "Substitute the given angle and the angle just found."], ["∠ABD + ∠CDB = 180°", "Simplify."], ["∴ AB // CE", reasonRef("parallel.converseCoInterior", {"params":{"line1":"AB","line2":"CE"},"variant":"textbook2"})]]} />
      <div className="note"><strong>Notice the change of reason.</strong> The first step uses a given parallel pair. The final step proves a different pair parallel using <strong><ReasonContent>{reasonRef("parallel.converseCoInterior", {"params":{"line1":"AB","line2":"CE"},"variant":"textbook1"})}</ReasonContent></strong>.</div>
    </section>
    <section className="lessonCard">
      ...
      <details className="solution"><summary>Reveal the alternate-angle proof</summary><div className="solutionBody"><Proof caption="Establish equality before claiming parallelism" rows={[["a = 2(22) + 13 = 57°", "Substitute x = 22 (given)."], ["b = 3(22) − 9 = 57°", "Substitute x = 22 (given)."], ["a = b", "Both angles equal 57°."], ["∴ p // q", reasonRef("parallel.converseAlternate", {"params":{"line1":"p","line2":"q"},"variant":"textbook2"})]]} /></div></details>
