...
import { s2Chapter6Flashcards } from "@/data/maths/flashcards/s2chapter6";

export default function S2Chapter6FlashcardsPage() {
  const [cards, setCards] = useState<typeof s2Chapter6Flashcards | null>(null);
  const [selected, setSelected] = useState<string[]>(Array.from(new Set(s2Chapter6Flashcards.map(card => card.section))));
  const sections = Array.from(new Set(s2Chapter6Flashcards.map(card => card.section)));
  const available = s2Chapter6Flashcards.filter(card => selected.includes(card.section));
  ...
  return <main className="page">
    ...
    <header><p className="eyebrow">S2 · CHAPTER 6 · FLASHCARDS</p><h1>Angles and Parallel Lines (II)</h1><p>Recall the rules, interpret the diagrams and explain each proof.</p></header>
    {cards ? <><div className="sessionToolbar"><button type="button" onClick={() => setCards(null)}>← Change sections</button><span>{cards.length} cards in this session</span></div><FormulaFlashcards key={cards.map(card => card.id).join(",")} cards={cards} /></> : <section className="selection">
      ...
      <div className="options">{sections.map((section, index) => <label key={section}><input type="checkbox" checked={selected.includes(section)} onChange={() => setSelected(current => current.includes(section) ? current.filter(item => item !== section) : [...current, section])} /><span><strong>Section {index + 1} · {section}</strong><small>{s2Chapter6Flashcards.filter(card => card.section === section).length} flashcards</small></span></label>)}</div>
      ...
      <div className="links"><Link href="/maths/flashcards?level=s2&chapter=6">Choose across levels and chapters →</Link><Link href="/maths/flashcards/review">Start Smart Review →</Link><Link href="/maths/flashcards/progress">View my progress →</Link></div>
    </section>}
    <style jsx>{`
