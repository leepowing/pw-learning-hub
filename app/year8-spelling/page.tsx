...
const availableWeekNumbers = Object.keys(year8SpellingWeeks).map(Number);
...
const totalWords = availableWeekNumbers.reduce(
  (total, weekNumber) =>
    total + (year8SpellingWeeks[weekNumber]?.words.length ?? 0),
  0
);
...
const completedWeeks = availableWeekNumbers.filter(
  (weekNumber) => {
    const totalWeekWords =
      year8SpellingWeeks[weekNumber]?.words.length ?? 0;
...
<section className="review-centre-card">
  ...
  {reviewCount > 0 ? (
    <Link href="/year8-spelling/review" className="review-button">
      Start Review →
    </Link>
  ) : (
    <span className="review-complete">
      ✅ All caught up
        ...
        <div className="week-grid">
            ...
            const weekData = year8SpellingWeeks[weekNumber];
            ...
            return (
              <Link
                key={weekNumber}
                href={`/year8-spelling/${weekNumber}`}
